/* ═══════════════════════════════════════════════════════
   ...
   Dev: fetch YAML from dev server
   Prod: fetch JSON from /data/ (converted at build time)
   ═══════════════════════════════════════════════════════ */
...
/**
 * Fetch and parse a single YAML/JSON file, with caching
 */
export async function loadYaml(path) {
    ...
    let fetchPath = path;
    if (!isDev) {
        // In production, YAML files are converted to JSON
        fetchPath = path.replace(/\.yaml$/, '.json');
    }
